...
void setSuccess(int flag) {
    successFlag = flag;
    successTime = TCPIP_SNTP_UTCSecondsGet();
}

int getSuccess(){
    if((successFlag == 0) ||(TCPIP_SNTP_UTCSecondsGet() - successTime) >= 1){
        successFlag = 0;
        return 0;
    }
    else{
...
uint8_t TCPIP_HTTP_FileAuthenticate(HTTP_CONN_HANDLE connHandle, uint8_t *cFile)
{
    ...
    if(memcmp(cFile, (const void *)"", 0) == 0)
...
HTTP_IO_RESULT TCPIP_HTTP_GetExecute(HTTP_CONN_HANDLE connHandle)
{
    ...
    else if(!memcmp(filename, "btns.cgi", 8))
    {
        // Determine which LED to toggle.
        ptr = TCPIP_HTTP_ArgGet(httpDataBuff, (const uint8_t *)"btn");
    ...
    else if(!memcmp(filename, "anCoefs.cgi", 11))
    {
        ...
        int i;
        // Determine which LED to toggle.
        ptr = TCPIP_HTTP_ArgGet(httpDataBuff, (const uint8_t *)"val1");
        for(i=0; i<9; i++){
            bufStrs[0][i] = *ptr;
            if(*ptr == '\0'){
                break;
...
void TCPIP_HTTP_Print_led0(HTTP_CONN_HANDLE connHandle)     //ledAlarmStatus
{
    if(getAlert() == 1)
    {
...
void TCPIP_HTTP_Print_led6(HTTP_CONN_HANDLE connHandle)     //ledWater1
{
    if(getWater(0) == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_led7(HTTP_CONN_HANDLE connHandle)     //ledWater2
{
    if(getWater(1) == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_led8(HTTP_CONN_HANDLE connHandle)     //ledWater3
{
    if(getWater(2) == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_led9(HTTP_CONN_HANDLE connHandle)     //ledWater4
{
    if(getWater(3) == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_led10(HTTP_CONN_HANDLE connHandle)        //ledSmoke1
{
    if(getSmoke(0) == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_led11(HTTP_CONN_HANDLE connHandle)        //ledSmoke2
{
    if(getSmoke(1) == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_led12(HTTP_CONN_HANDLE connHandle)        //ledSmoke3
{
    if(getSmoke(2) == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_led13(HTTP_CONN_HANDLE connHandle)        //ledSmoke4
{
    if(getSmoke(3) == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_led14(HTTP_CONN_HANDLE connHandle)        //ledAlarmEnabled
{
    if(getAlarm() == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_led15(HTTP_CONN_HANDLE connHandle)        //ledPump1
{
    if(getPump(0) == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_led16(HTTP_CONN_HANDLE connHandle)        //ledPump2
{
    if(getPump(1) == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_led17(HTTP_CONN_HANDLE connHandle)        //ledPLC
{
    if(getPLC() == 1)
    {
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), "on");
...
void TCPIP_HTTP_Print_pot0(HTTP_CONN_HANDLE connHandle)     //Temp 1
{
    uint8_t AN0String[8];
    int temp = readTemp(0);

    itoa((uint8_t *)AN0String, temp, 10);

    TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_pot1(HTTP_CONN_HANDLE connHandle)     //Temp 2
{
    uint8_t AN0String[8];
    int temp = readTemp(1);
...
void TCPIP_HTTP_Print_pot2(HTTP_CONN_HANDLE connHandle)     //Temp 3
{
    uint8_t AN0String[8];
    int temp = readTemp(2);
...
void TCPIP_HTTP_Print_pot3(HTTP_CONN_HANDLE connHandle)     //Temp4
{
    uint8_t AN0String[8];
    int temp = readTemp(3);
...
void TCPIP_HTTP_Print_offs(HTTP_CONN_HANDLE connHandle, uint16_t num)       //EventLog1
{
...
void TCPIP_HTTP_Print_warn(HTTP_CONN_HANDLE connHandle, uint16_t num)       //EventLog1
{
...
void TCPIP_HTTP_Print_successFlag(HTTP_CONN_HANDLE connHandle, uint16_t num)       //EventLog1
{
    uint8_t SuccessString[8];
